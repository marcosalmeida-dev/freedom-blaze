...
/// <summary>
/// <see cref="INewsStore"/> backed by the local SQLite database (EF Core). The generated news
...
/// multi-entity transaction that must run in a single context.
/// </para>
/// </summary>
public class SqliteNewsStore : INewsStore
{
    private readonly IDbContextFactory<FreedomBlazeDbContext> _contextFactory;
    private readonly TimeProvider _timeProvider;
    private readonly ILogger<SqliteNewsStore> _logger;

    public SqliteNewsStore(
        IDbContextFactory<FreedomBlazeDbContext> contextFactory,
        TimeProvider timeProvider,
        ILogger<SqliteNewsStore> logger)
    {
    ...
    public async Task SaveAsync(DateOnly date, IReadOnlyList<NewsArticleModel> articles, string? model = null, CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogWarning(ex, "Failed to persist Bitcoin news to SQLite for {Date}.", date);
        }
    }
