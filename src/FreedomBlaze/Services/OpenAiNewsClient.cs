...
public class OpenAiNewsClient
{
    ...
    private readonly ILogger<OpenAiNewsClient> _logger;
    private readonly ResponsesClient? _responses;

    private static readonly JsonSerializerOptions JsonOptions = new(JsonSerializerDefaults.Web);

    public OpenAiNewsClient(
        ...
        OpenAIClient? openAiClient = null)
    {
        ...
        // The OpenAIClient is the SDK's recommended entry point; derive the per-feature
        // ResponsesClient from it. It is null only when no API key is configured, in which
        // case calls throw a clear error instead of a NullReferenceException.
        _responses = openAiClient?.GetResponsesClient();
    }
